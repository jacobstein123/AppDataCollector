...
def press_key(key):
    win32api.keybd_event(key, 0, 0)
    win32api.keybd_event(key, 0, win32con.KEYEVENTF_KEYUP, 0)

def shift(key):
    win32api.keybd_event(0xA0, 0, 0)
    press_key(key)
...
def get_tuple_difference(t1, t2):
    difference = 0
    difference += abs(t1[0]-t2[0])
    difference += abs(t1[1]-t2[1])
    difference += abs(t1[2]-t2[2])
    return difference
...
def download_app(package_name):
    ...
    print 'You Have 5 Seconds To Open Chrome In Fullscreen!'
    time.sleep(5)
    message = "https://play.google.com/store/apps/details?id="+package_name
    ...
    #find button based on color signature:
    for i in range(500,2000,2): #scan for the extension button
        win32api.SetCursorPos((i,43))
        if get_tuple_difference(get_pixel_color(i,43),(147, 180, 31)) < 100:
            click(i,43) #click the extension to download the app
            break